...
export default function Header({user}) {
    ...
    const skillLevel = (numberChords)=> {
        if (numberChords<4) {return 'just started'};
        if (numberChords<8) {return 'beginner'};
        if (numberChords<12) {return 'ready for a jam'};
        if (numberChords<15) {return 'just started'}
        else return 'pro'
    }
    ...
    return (
        ...
        <header className="App-header">
          <div className='head-user neon'>
            ...
            <div className="head-username neon-red delay">
            {user.name}!
            </div>
          </div>
